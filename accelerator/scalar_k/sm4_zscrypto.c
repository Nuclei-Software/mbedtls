...
void    sm4_key_schedule_enc (
    ...
    unsigned long K0 = mkp[0] ^ FK[0];
    unsigned long K1 = mkp[1] ^ FK[1];
    unsigned long K2 = mkp[2] ^ FK[2];
    unsigned long K3 = mkp[3] ^ FK[3];
...
void    sm4_block_enc_dec (
    ...
    unsigned long   X0  = (inp[0]);
    unsigned long   X1  = (inp[1]);
    unsigned long   X2  = (inp[2]);
    unsigned long   X3  = (inp[3]);
