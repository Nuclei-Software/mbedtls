...
#if defined(MBEDTLS_SHA1_DMA_ALT)
#define mbedtls_internal_sha1_process_many            mbedtls_internal_sha1_process_many_dma_alt
#define mbedtls_internal_sha1_process_once            mbedtls_internal_sha1_process_dma_alt
#else
#define mbedtls_internal_sha1_process_many            mbedtls_internal_sha1_process_many_alt
#define mbedtls_internal_sha1_process_once            mbedtls_internal_sha1_process_alt
#endif
...
int mbedtls_internal_sha1_get_hmac_result(mbedtls_sha1_context *ctx, unsigned char *out)
{
    ...
    if(ret == 0) {
        ...
        HASH_GetResult_REV(HASH0, output, 5);
    }
    return( 0 );
}
#endif

static int mbedtls_internal_sha1_process_dma_alt( mbedtls_sha1_context *ctx,
                                    const uint8_t *data,
                                    uint32_t curlen,
                                    uint32_t hashHandleState )
{
    ...
    if (waitState == 1) {
        ...
        if(ret == 0) {
            ...
            HASH_GetResult(HASH0, output, 5);
        }
    }
    return( 0 );
}

static int mbedtls_internal_sha1_process_alt( mbedtls_sha1_context *ctx,
...
static size_t mbedtls_internal_sha1_process_many_alt(
                  mbedtls_sha1_context *ctx, const uint8_t *data, size_t len )
{
    size_t processed = 0;

    while( len >= SHA1_BLOCK_SIZE ) {
        if( mbedtls_internal_sha1_process_once( ctx, data, SHA1_BLOCK_SIZE, MD_SEGMENT_DOWN ) != 0)
            return( 0 );

        data += SHA1_BLOCK_SIZE;
        len  -= SHA1_BLOCK_SIZE;
        processed += SHA1_BLOCK_SIZE;
...
static size_t mbedtls_internal_sha1_process_many_dma_alt(
                  mbedtls_sha1_context *ctx, const uint8_t *data, size_t len)
{
    size_t processed = 0;
    uint32_t blockNum = (uint32_t)((len / SHA1_BLOCK_SIZE) * SHA1_BLOCK_SIZE);

    while( len >= SHA1_BLOCK_SIZE ) {
        if( mbedtls_internal_sha1_process_once( ctx, data, blockNum, MD_SEGMENT_DOWN ) != 0)
            return( 0 );
...
int mbedtls_sha1_update_key( mbedtls_sha1_context *ctx,
                               ...
                               uint8_t keystate)
{
    ...
    if (keystate == HMAC_SET_FST_KEY) {
        if (!isFstConfiged) {
            isFstConfiged = 1;
            /* HASH Configuration */
            HASH_InitStructure.HASH_AlgoSelection = HASH_RF_CTRL_ALGO_SHA_1;
            ...
            HASH_SegLenInitStruct.HASH_Seg_CurrLen = SHA1_BLOCK_SIZE;
            HASH_Ip_Init(HASH0);
    ...
    if( ( ret = mbedtls_internal_sha1_process_alt( ctx, key, SHA1_BLOCK_SIZE, hashDownState ) ) != 0 )
        return( ret );
                               ...
                               size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        if( ( ret = mbedtls_internal_sha1_process_once( ctx, ctx->buffer, SHA1_BLOCK_SIZE, MD_SEGMENT_DOWN ) ) != 0 )
            return( ret );
    ...
    blockNum = (uint32_t)((ilen / SHA1_BLOCK_SIZE) * SHA1_BLOCK_SIZE);
    ...
    while( ilen >= SHA1_BLOCK_SIZE )
    {
        ...
        processed = mbedtls_internal_sha1_process_many( ctx, input, ilen );

        if( processed < SHA1_BLOCK_SIZE )
                               ...
                               unsigned char *output )
{
    ...
    if( ( ret = mbedtls_internal_sha1_process_once( ctx, ctx->buffer, used, hashDownState ) ) != 0 )
        return( ret );
